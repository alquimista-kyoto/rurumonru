...
// 各記事のページでパーマリンク文字列を入力する際、スラッシュがハイフンに自動変換されてしまうのを防ぐ
...
/* リライトルールを書き換える。*/
function my_rewrite_rules_array($my_rewrite) {
 
	$return_rule = array();
	
	foreach ($my_rewrite as $regex => $rewrite){
 
		// スラッシュをはじかないルールに書き換える。
...
get_template_part( 'include/breadcrumb' );
